...
void register_connection(int fd, const std::string& ip, int port) {
    ...
    connections[fd] = { fd, ip, port, "active",
...
void console_thread() {
    ...
    while (server_running && std::getline(std::cin, line)) {
        if (line == "list") {
            ...
            std::cout << "[console] Queue depth: " << work_queue.size()
                      << " | Connections tracked: ";
            {
                std::lock_guard<std::mutex> lk(conn_mutex);
...
int main() {
    ...
    while (server_running) {
        ...
        std::cout << "[listener] Accepted " << ip_str << ":" << port
                  << " → fd=" << client_fd << "\n";
